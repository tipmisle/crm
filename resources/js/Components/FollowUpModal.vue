...
<template>
    <Modal :show="show" max-width="sm" @close="emit('close')">
        <form class="p-6" @submit.prevent="submit">
            <h2 class="text-base font-semibold text-neutral-900">Set a follow-up</h2>

            <div class="mt-4 space-y-4">
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-neutral-700">Note</label>
                    <textarea
                        v-model="form.note"
                        rows="2"
                        placeholder="Follow up about the quote…"
                ...
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-neutral-700">Due</label>
                    <input
                        v-model="form.due_at"
                        type="datetime-local"
                        class="w-full rounded-md border border-neutral-200 px-3 py-2 text-sm outline-none focus:border-neutral-400"
