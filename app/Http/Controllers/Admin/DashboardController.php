    ...
    public function index(): Response
    {
        return Inertia::render('Admin/Dashboard', [
            'stats' => [
                ...
                'instagram_connected' => Integration::withoutGlobalScopes()->where('provider', IntegrationProvider::Meta->value)->where('status', 'connected')->whereHas('channels', fn ($q) => $q->where('type', 'instagram')->where('status', 'connected'))->count(),
                'facebook_connected' => Integration::withoutGlobalScopes()->where('provider', IntegrationProvider::Meta->value)->where('status', 'connected')->whereHas('channels', fn ($q) => $q->where('type', 'facebook_messenger')->where('status', 'connected'))->count(),
